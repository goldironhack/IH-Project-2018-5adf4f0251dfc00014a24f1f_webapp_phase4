  ...
  var data = $.get(HousingNY_URL,function(){})
    .done(function(){
		...
		for (var j = 0; j < housing.length; j++) {
            latitudeString = dataRow[j][25];
			longitudString = dataRow[j][26];
      ...
      //alert("latitude " + typeof latitude + " " + latitude);
			...
			var housingName = dataRow[j][9];
			var housingPoint = {lat: latitude, lng: longitud};
			var lowIncome = dataRow[j][31];
...
$("document").ready(function(){
  $("#getHousingNY").on("click", getHousingNY)
})
/*
